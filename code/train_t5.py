...
from overrides_new import DataCollatorForLanguageModeling, DoNothingDataCollator, DoNothingDataCollatorForGeneration,T5ForConditionalGenerationCustom
...
def main():
    ...
    if model_args.model_name_or_path != "new":
        ...
        model = T5ForConditionalGenerationCustom(config=config)
    ...
    if training_args.do_eval:
        ...
        output_eval_file = os.path.join(training_args.output_dir, data_args.save_file+".txt")
        ...
        for inputs in tqdm(data_loader, "Prediction"):
            ...
            with torch.no_grad():
                outputs,possitive_score,negative_score = model.generate(
                    ...
                    max_length=12
                ) #outputs,possitive_score,negative_score
                #
                #print(outputs)
